/*
 * BandPassFilterController — Hamation MBF-100 BandPasser II
 *
 * Watches one radio over TCP (Kenwood IF;) and drives 6 control lines that
 * gate the +5–12 V band-select inputs on the Hamation rear panel (via the
 * 8-relay carrier; see ../docs/HARDWARE.md). WARC bands, out-of-band, parse
 * failures, and disconnects all fall through to bypass (all 6 outputs LOW —
 * the BandPasser II auto-bypasses with no input applied).
 *
...
constexpr const char* kDefaultHostname = "bpf-hamation";
constexpr const char* kFilterLabel     = "Hamation BandPasser II";
...
void setup() {
  Serial.begin(115200);
  Serial.println();
  Serial.println(F("BandPassFilterController :: Hamation BandPasser II"));
