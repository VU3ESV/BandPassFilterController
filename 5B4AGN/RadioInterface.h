...
// One instance per controller. Non-blocking tick() drives reconnect with
// bounded backoff (1→2→4→8 s) and exposes the most recently parsed frequency.
...
public:
  ...
  // Returns true if a fresh frequency was decoded this tick.
  bool tick() {
    uint32_t now = millis();
